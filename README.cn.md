...
- 前端响应式布局完善与适配更多终端尺寸。
- 提供 Android（原生 HTTP，支持后端配置与自动跨域）。
...
- 提供 iOS 壳程序（WebView + 原生桥接），支持离线缓存与文件导入
...
- 在 `slam_web/.env` 中设置后端地址：
  ```bash
  MODERN_PUBLIC_API_BASE=https://<你的后端域名或IP>/api
  ```
- Android 原生环境（Capacitor）下，前端会读取该变量并使用原生 HTTP 适配器发起请求，自动绕过浏览器 CORS 限制（`slam_web/src/services/http.ts:11`，`slam_web/src/services/axiosCapacitorAdapter.ts:23`）。
- 未设置时默认 `baseURL='/api'`，依赖开发代理或 Nginx 反向代理。
